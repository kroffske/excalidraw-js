...
- If the package or CLI is not already installed or is not reachable through `PATH`, stop and tell the user to run setup or add the npm/global Node bin directory to `PATH`, for example `npm install @kroffske/excalidraw-diagrams` in the current workspace, `npx -y @kroffske/excalidraw-diagrams install --agent agents --force` for a user-level Pi/global CLI setup, or `export PATH="$(npm config get prefix)/bin:$PATH"` for the active global npm prefix. Do not perform the install yourself unless the user explicitly asks for setup.
